...
public class EventController   {
	...
	public ResponseEntity<String> ajout(@ModelAttribute Event event,@PathVariable("idkinder") int kinderId,@PathVariable("iduser") int userId ,@RequestParam("file") MultipartFile file){
   ...
   //http://localhost:8081/SpringMVC/servlet/Eventname
	...
	@ResponseBody
	public ResponseEntity<String> updateEvent(@ModelAttribute Event e, @PathVariable("idEvent")int idEvent ,@RequestParam("file") MultipartFile file) {
  ...
  //http://localhost:8081/SpringMVC/servlet/retrievealleventsoftoday
   ...
   //http://localhost:8081/SpringMVC/servlet/retrieve-all-eventsordonnebydate
		...
		@PostMapping("/assiAttachementToPost/{id}")
		public void assiAttachementToPost(@PathVariable("id") int id , @RequestParam("file") MultipartFile file){
			if (uploadFileService.addFile(file)){
				ieventservice.assiAttachementToPost(id, file);
			}
		}		
