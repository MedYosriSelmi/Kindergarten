...
public interface MessageRepository extends CrudRepository<Message, Integer> {
	...
	@Query("Select m.Description from Message m "
			+ "where (m.Sender.id=:senderId and m.Reciever.id=:recieverId) "
			+ "OR (m.Sender.id=:recieverId and m.Reciever.id=:senderId) "
	...
	public Set<String> searchStringInDiscussion(@Param("string") String string, @Param ("senderId") int senderId, @Param("recieverId") int recieverId);
